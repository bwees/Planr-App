...
const ActivityList = (props) => {
    ...
    React.useEffect(() => {
        const refreshList = props.navigation.addListener('focus', () => updateActivityList(""));
    }, [props.navigation]);

    React.useEffect(() => {
        updateActivityList(filterText)
    }, [filterText])
        ...
        <View style={{ flex: 1 }}>
            <View style={[{ backgroundColor: colors.headerColor, zIndex: 100 }]}>
                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingTop: insets.top }}>
                    <Text style={[FONTS.h1, FONTS.bold, { color: colors.primary, marginHorizontal: 20, marginVertical: 8 }]}>Activities</Text>
